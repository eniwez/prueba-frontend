...
export default function ActionButtons({ onLike, onDislike, onHistory }: ActionButtonsProps) {
  return (
    <div className="mt-6 flex justify-center gap-6">
      ...
      >
        <ThumbsDown size={32} className="text-red-500 fill-red-500" />
      </button>
      ...
      >
        <History size={28} className="text-purple-500" />
      </button>
      ...
      <button
        ...
        className="w-20 h-20 rounded-full bg-white shadow-lg flex items-center justify-center
           hover:scale-110 transition"
      >
        <ThumbsUpIcon size={32} className="text-green-500 fill-green-500" />
      </button>
    </div>
  );
}
